 ...
 * @author Tony
 */
public class MyRecursiveSet implements AdvancedList
{
	/*
	 * This list implements a recursive linkedlist with a functionality of a set. Each addition to the linkedlist is checked if a duplicate
	 * element exists - in order to mimic the same ruling of a set. (This also complies to the requirement of recursive methods) 
	 * 
	 * This recursive linkedlist implements the same methods as our previous list EXCEPT our add and set methods, where the difference is
	 * in the validations if an existing element is already in the set before adding/updating.
	 * 
	 * Having the same tester class, we receive a few errors due to the different implementations.
	 * 		+ add/get/set/remove(object) - since our set does not accept similar items, the size of our set will not reflect the test cases
	 * 				that are being added (i.e. advancedValues[]). This causes index and object parameters to throw an error as it will
	 * 				reference to a duplicate object or an incorrect index/position.
    ...
    {
    	if(contains(e)) {
    		System.out.println("Unable to add due to duplicate values. " + e.toString() + " is already in our set.");
    		return false;
    	}
...
//    	System.out.println("tempNode.next is " + tempNode.next.item.toString() + " and index is " + index);
    ...
    public Object set(int index, Object element)
    {
    	...
    	if(contains(element)) {
    		System.out.println("Unable to add due to duplicate values. " + element.toString() + " is already in our set.");
    		return null;
    	}
