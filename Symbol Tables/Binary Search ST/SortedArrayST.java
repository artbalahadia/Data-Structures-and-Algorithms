...
public class SortedArrayST<Key extends Comparable<Key>, Value> {
	...
	public void put(Key key, Value val) {
		...
		vals[r] = val;           //  put/update val to key
	}
	...
	private void shiftRight(int r) {
		...
		if(N <= 0 || keys[N-1] != null) {
			resize(N+1);					// resize if array is empty or full
			N++;
		}
	...
	public Key floor(Key key) {	
		...
		if(x >= N || !keys[x].equals(key)) {
			return keys[x-1];
		} else if(keys[x].equals(key)) {
			return keys[x];
		}
	...
	public int countRange(Key key1, Key key2) {
		...
		for(count = 0; lo <= hi && lo < N; lo++) {
			if(lo == hi) {
				if(!keys[lo].equals(key1)) {
					if(!keys[lo].equals(key2)) {
						break;
					}
				}
			}
		...
		return count; // ToDo4:  implement this function
	}
	...
	public static void allShiftLeftTests() {
		...
		testDelete("HY","12","Y","H","1");				   // 1.1 delete from 2 keys the last value
		testDelete("","","","","");						   // 1.2 delete empty array
	...
	public static void allShiftRightTests() {
		...
		testPut("LMNO", "1234", "P","5", "LMNOP","12345");	// 2.1 put last
		testPut("","","X","1","X","1");						// 2.2 put to an empty array
	...
	public static void allCountRangeTests() {
		...
		testCountRange("DJPQV","13456", "K","R",2);    // partial Range
